...
class DuplicateDetectionAlgorithm(ABC):
    ...
    def compute_distance_matrix(self) -> None:
        ...
        for i in range(num_samples):
            for j in range(i + 1, num_samples):
                distance = self.calculate_distance(self.dataset_array[i], self.dataset_array[j])
    ...
    def get_new_distance_matrix_column(self) -> np.ndarray:
        ...
        for i in range(num_samples):
            new_distances[i] = self.calculate_distance(self.input_vector, self.dataset_array[i])
    ...
    def add_new_vector_to_distance_matrix(self) -> None:
        ...
        self.distance_matrix = np.hstack((self.distance_matrix, new_distances[:, np.newaxis]))
