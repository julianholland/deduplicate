...
class MultiHashing(DuplicateDetectionAlgorithm):
    def __init__(
        ...
        super().__init__(tolerance, input_vector, dataset_array, distance_metric, distance_matrix)
    ...
    def create_perturbation_array(self, override: bool = False) -> None:
        if self.perturbation_array is not None and not override:
            print("Perturbation array already exists. Skipping creation. To override, set override=True.")
        else:
            self.perturbation_array = np.abs(np.random.normal(loc=0.0, scale=1.0, size=self.perturbations))
    
    def round_to_tolerance(self, input_global_descriptor: np.ndarray) -> np.ndarray:
        rounded_vector = np.round(input_global_descriptor / self.tolerance) * self.tolerance
        return rounded_vector
    
    def create_hash_vector(self, input_global_descriptor: np.ndarray, perturbation_array: np.ndarray = None) -> np.ndarray:
        ...
        for i in range(self.perturbations):
            hash_vector[i] = hash(input_global_descriptor*perturbation_array[i].tobytes())
        return hash_vector
    ...
    def create_hash_vector_array(self, dataset_array: np.ndarray) -> np.ndarray:
        ...
        for i in range(dataset_array.shape[0]):
            hash_vector_array[i] = self.create_hash_vector(dataset_array[i], perturbation_array)
    ...
    def compute_clash_array(self) -> np.ndarray:
        perturbation_array = self.create_perturbation_array()
        clash_array= np.zeros((self.perturbations, self.dataset_array.shape[0]), dtype=bool)
        for i in range(self.dataset_array.shape[0]):
            perturbed_array = self.round_to_tolerance(self.dataset_array[i]) * perturbation_array
